...
async def get_latest_decision(db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"error": f"Failed to get latest decision: {str(e)}"}
...
async def get_panel_history(panel_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"error": f"Failed to retrieve history for panel {panel_id}: {str(e)}", "panel_id": panel_id}
...
async def get_system_logs(db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"error": f"Failed to retrieve system logs: {str(e)}"}
