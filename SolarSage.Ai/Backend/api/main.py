...
async def get_panel_history(panel_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"error": f"Failed to retrieve panel history: {str(e)}", "panel_id": panel_id}
...
async def get_system_logs(db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"error": f"Failed to retrieve system logs: {str(e)}", "logs": []}
...
async def get_system_stats(db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"error": f"Failed to retrieve system stats: {str(e)}", "total_panels": 0}